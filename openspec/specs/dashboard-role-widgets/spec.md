...
#### Scenario: analyst 角色看到分析部門 Widget
...
- **THEN** 計算邏輯為：最新 risk_assessment 中 axis1_score、axis2_score 或 axis3_score 任一 >= 60 的供應商數量
- **AND** 舊的 probability × impact 計算方式 MUST NOT 再使用

### Requirement: 永續風險概覽側欄入口

#### Scenario: 所有風險相關角色可從側欄進入永續風險概覽
- **WHEN** admin、sustain、comply、analyst 角色的使用者查看側欄
- **THEN** 「風險稽核」子選單 MUST 包含「永續風險概覽」連結
