...
系統 SHALL 提供 `GET /api/v1/risk/six-dim-heatmap` 端點，回傳所有有風險評估記錄的供應商與其最新 E1–E6 六維分數，供熱圖視覺化使用。

#### Scenario: 正常回傳熱圖資料
- **WHEN** 已認證用戶發送 `GET /api/v1/risk/six-dim-heatmap`
- **THEN** 回傳每家供應商的最新 RA（以 `assessed_at` 取最新）
- **AND** 回應包含：supplier_id、supplier_name、supplier_code、country_code、assessed_at、source_type、dim_e1–dim_e6（null 代表未評估）、open_cap_count、risk_score
- **AND** 回應包含 `thresholds` 物件（從 system_settings.cap_thresholds 讀取）
- **AND** 回應包含 `summary.total`（有 RA 的供應商總數）及 `summary.any_dim_critical`（任一維度低於閾值的供應商數）

#### Scenario: 無供應商有 RA 時回傳空陣列
- **WHEN** 資料庫中無 risk_assessments 記錄
