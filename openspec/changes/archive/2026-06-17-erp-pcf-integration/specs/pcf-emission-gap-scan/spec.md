...
系統 SHALL 提供 `POST /api/v1/buyer-products/{id}/bom-lines/{lineId}/request-emission` endpoint，允許 buyer / sustain 角色對特定 BOM 行手動建立 PcfRequest（`trigger_source = 'buyer_manual'`）。
...
- **WHEN** 該 (material_item × supplier) 已有 status = pending 的 PcfRequestLine
- **THEN** 系統 SHALL 回傳 409，說明已有待填報請求
