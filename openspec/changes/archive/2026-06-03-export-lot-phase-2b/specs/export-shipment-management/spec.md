...
- 建立 Shipment 時，依 ShipmentLine 的 TradeGood.is_eudr_applicable 自動決定 `eudr_dds_status`：有任一 EUDR 適用商品則為 `draft`，否則為 `not_required`
- `shipment_line_batches` 的 (shipment_line_id, production_batch_id) 為 UNIQUE

#### Scenario: 自動 EUDR 狀態判斷
- **WHEN** 新增 ShipmentLine 且其 TradeGood.is_eudr_applicable = true
...
- 詳情頁（`/compliance/shipments/:id`）：
  ...
  - 「分配批號」Modal：依 trade_good_id 篩選可用 ProductionBatch，輸入 allocated_quantity
